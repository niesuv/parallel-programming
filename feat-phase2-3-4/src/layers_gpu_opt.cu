/**
 * @file layers_gpu_opt.cu
 * @brief GPU Layer Implementations (Optimized Version - Phase 3)
 *
 * This file contains optimized CUDA implementations for neural network layers.
 * Enabled by compiling with -DUSE_OPTIMIZED_KERNELS
 *
 * Optimizations applied:
 * 1. Shared Memory Tiling - Reduce global memory bandwidth
 * 2. Constant Memory - Cache small weight tensors
 * 3. Memory Coalescing - Aligned memory access patterns
 * 4. Warp Shuffle Reduction - Fast parallel reduction using __shfl_down_sync
 * 5. Loop Unrolling - #pragma unroll for 3x3 kernels
 * 6. Vectorized Loads - float4 for ReLU operations
 * 7. Fused Kernels - Conv2D + ReLU in single kernel
 *
 * Expected speedup vs naive (layers_gpu.cu):
 * - Conv2D: 2-4x (tiling + constant memory)
 * - MSE Reduction: 3-5x (warp shuffle)
 * - MaxPool2D: 1.5-2x (2D blocks + unrolling)
 */
...
// =============================================================================
// Constants for Tiled Convolution
// =============================================================================
...
// Constant memory for small weight tensors (avoids global memory reads)
__constant__ float c_weights_small[8192];
__constant__ float c_bias_small[256];

// =============================================================================
// Fused Conv2D + ReLU Kernel (Most Important Optimization)
// =============================================================================
    ...
    int stride, int padding, bool use_const)
{
      ...
      const int num_loads =
          (tile_size + threads_per_block - 1) / threads_per_block;
...
#pragma unroll 4
        for (int load = 0; load < num_loads; ++load) {
          ...
          if (linear_idx < tile_size) {
            ...
            if (sw < tile_w && ih >= 0 && ih < in_h && iw >= 0 && iw < in_w) {
              s_input[sh * (tile_w + PAD) + sw] =
                  input[in_channel_offset + ih * in_w + iw];
            } else {
        ...
        const size_t w_ic_offset =
            (static_cast<size_t>(oc) * in_c + ic) * k * k;
...
#pragma unroll
      ...
      // Fused ReLU
      output[((static_cast<size_t>(n) * out_c + oc) * out_h + oh) * out_w +
             ow] = fmaxf(0.0f, sum);
    }

    // Declare external kernel from layers_gpu.cu
    extern __global__ void relu_forward_kernel(const float *input,
...
#pragma unroll 4
        ...
        for (int kh = 0; kh < k; ++kh) {
          for (int kw = 0; kw < k; ++kw) {
            ...
            if (ih >= 0 && ih < in_h && iw >= 0 && iw < in_w) {
              size_t in_idx =
                  ((static_cast<size_t>(n) * in_c + ic) * in_h + ih) * in_w +
                  iw;
              size_t w_idx =
    ...
    __global__ void
    maxpool2d_forward_opt_kernel(const float *__restrict__ input,
...
#pragma unroll
        for (int kw = 0; kw < k; ++kw) {
          ...
          if (ih < in_h && iw < in_w) {
            size_t in_idx =
                ((static_cast<size_t>(n) * channels + c) * in_h + ih) * in_w +
                iw;
    ...
    __global__ void
    upsample2d_forward_opt_kernel(const float *__restrict__ input,
                                  float *__restrict__ output, int batch_size,
                                  int channels, int in_h, int in_w, int out_h,
                                  int out_w, int scale) {
      int ow = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    void gpu_relu_forward_opt(const GPUTensor4D &input, GPUTensor4D &output,
      ...
      if (n % 4 == 0) {
        ...
        relu_forward_kernel<<<grid_size, block_size, 0, stream>>>(
            input.d_data, output.d_data, n);
      }
    ...
    void gpu_maxpool2d_forward_opt(const GPUTensor4D &input,
                                   GPUTensor4D &output, int k, int stride,
                                   cudaStream_t stream) {
      ...
      dim3 grid((out_w + block.x - 1) / block.x,
                (out_h + block.y - 1) / block.y, input.n * input.c);

      maxpool2d_forward_opt_kernel<<<grid, block, 0, stream>>>(
          input.d_data, output.d_data, input.n, input.c, input.h, input.w,
          out_h, out_w, k, stride);
      CUDA_CHECK(cudaGetLastError());
    }

    void gpu_upsample2d_forward_opt(const GPUTensor4D &input,
      ...
      upsample2d_forward_opt_kernel<<<grid, block, 0, stream>>>(
          input.d_data, output.d_data, input.n, input.c, input.h, input.w,
          out_h, out_w, scale);
      CUDA_CHECK(cudaGetLastError());
    }
    ...
    void gpu_conv2d_relu_forward_opt(const GPUTensor4D &input,
      ...
      if (out_c <= 256 && in_c * k * k * out_c <= 8192) {
        CUDA_CHECK(cudaMemcpyToSymbol(c_weights_small, d_weights,
                                      in_c * k * k * out_c * sizeof(float)));
        CUDA_CHECK(
            cudaMemcpyToSymbol(c_bias_small, d_bias, out_c * sizeof(float)));
        use_const = true;
      }
      ...
      conv2d_bias_relu_fused_kernel<<<grid, block, shared_size, stream>>>(
          input.d_data, d_weights, d_bias, output.d_data, input.n, in_c,
          input.h, input.w, out_c, out_h, out_w, k, stride, padding, use_const);
      CUDA_CHECK(cudaGetLastError());
    }

    void gpu_conv2d_forward_tiled(const GPUTensor4D &input,
      ...
      conv2d_forward_tiled_kernel<<<grid, block, shared_size, stream>>>(
          input.d_data, d_weights, d_bias, output.d_data, input.n, in_c,
          input.h, input.w, out_c, out_h, out_w, k, stride, padding);
      CUDA_CHECK(cudaGetLastError());
    }
    ...
    __global__ void conv2d_backward_data_opt_kernel(
        ...
        const float *__restrict__ weights, float *__restrict__ grad_input,
        int batch_size, int in_c, int in_h, int in_w, int out_c, int out_h,
        int out_w, int k, int stride, int padding) {
      ...
      if (k == 3 && stride == 1) {
        for (int oc = 0; oc < out_c; ++oc) {
          ...
          // Optimize memory access: process kernel in order for better L2 cache
          // locality
          for (int kh = 0; kh < 3; ++kh) {
        ...
        for (int oc = 0; oc < out_c; ++oc) {
          const size_t go_oc_offset = go_base + oc * out_h * out_w;
          const size_t w_oc_base =
              (static_cast<size_t>(oc) * in_c + ic) * k * k;

          for (int kh = 0; kh < k; ++kh) {
            int oh_check = ih + padding - kh;
            if ((oh_check % stride == 0) && oh_check >= 0 &&
                oh_check < out_h * stride) {
              ...
              for (int kw = 0; kw < k; ++kw) {
                ...
                if ((ow_check % stride == 0) && ow_check >= 0 &&
                    ow_check < out_w * stride) {
    ...
    // Optimized conv2d backward weights kernel - parallel over weight elements
    // with better memory access
    __global__ void conv2d_backward_weights_opt_kernel(
        ...
        int in_w, int out_c, int out_h, int out_w, int k, int stride,
        int padding) {
...
#pragma unroll 4
            ...
            for (int ow = 0; ow < out_w; ++ow) {
              ...
              if (ih >= 0 && ih < in_h && iw >= 0 && iw < in_w) {
                size_t in_idx =
                    ...
                    iw;
                size_t go_idx =
                    ((static_cast<size_t>(n) * out_c + oc) * out_h + oh) *
                        out_w +
                    ow;
    ...
    // Optimized bias gradient with improved parallel reduction and better
    // memory access
    __global__ void conv2d_backward_bias_opt_kernel(
      ...
      for (int i = tid; i < total_spatial; i += blockDim.x) {
        ...
        size_t idx =
            ((static_cast<size_t>(n) * out_c + oc) * out_h * out_w) + spatial;
        sum += grad_output[idx];
      }
    ...
    __global__ void conv2d_relu_backward_data_opt_kernel(
      ...
      float sum = 0.0f;
      const size_t gi_idx =
          ((size_t)n * in_c + ic) * in_h * in_w + ih * in_w + iw;
...
#pragma unroll
              for (int kw = 0; kw < 3; ++kw) {
                ...
                if (ow >= 0 && ow < out_w) {
                  ...
                  float relu_grad =
                      (input[((size_t)n * in_c + ic) * in_h * in_w + (ih)*in_w +
                             iw] > 0.0f)
                          ? grad_output[go_row + ow]
                          : 0.0f;
    ...
    upsample2d_backward_opt_kernel(const float *__restrict__ grad_output,
                                   ...
                                   int in_w, int out_h, int out_w, int scale) {
      int iw = blockIdx.x * blockDim.x + threadIdx.x;
      int ih = blockIdx.y * blockDim.y + threadIdx.y;
      int c_n = blockIdx.z;
      int c = c_n % channels;
      ...
      size_t gi_idx = (static_cast<size_t>(n) * channels + c) * in_h * in_w +
                      ih * in_w + iw;
      grad_input[gi_idx] = sum;
    }
    ...
    void gpu_conv2d_backward_data_opt(const GPUTensor4D &grad_output,
      ...
      conv2d_backward_data_opt_kernel<<<grid, block, 0, stream>>>(
          grad_output.d_data, d_weights, grad_input.d_data, batch_size, in_c,
          in_h, in_w, out_c, grad_output.h, grad_output.w, k, stride, padding);
      CUDA_CHECK(cudaGetLastError());
    }
    ...
    void gpu_conv2d_backward_weights_opt(const GPUTensor4D &input,
      ...
      conv2d_backward_weights_opt_kernel<<<grid_size, block_size, 0, stream>>>(
          input.d_data, grad_output.d_data, d_grad_weights, input.n, in_c,
          input.h, input.w, out_c, grad_output.h, grad_output.w, k, stride,
          padding);
      CUDA_CHECK(cudaGetLastError());
      ...
      conv2d_backward_bias_opt_kernel<<<out_c, bias_block, shared_size,
                                        stream>>>(grad_output.d_data,
                                                  d_grad_bias, input.n, out_c,
                                                  grad_output.h, grad_output.w);
      CUDA_CHECK(cudaGetLastError());
    }

    void gpu_conv2d_relu_backward_data_opt(
        const GPUTensor4D &grad_output, const float *d_weights,
        const GPUTensor4D &input, GPUTensor4D &grad_input, int batch_size,
        int in_c, int in_h, int in_w, int out_c, int k, int stride, int padding,
        cudaStream_t stream) {
      dim3 block(16, 16);
      dim3 grid((in_w + block.x - 1) / block.x, (in_h + block.y - 1) / block.y,
                batch_size * in_c);
      ...
      conv2d_relu_backward_data_opt_kernel<<<grid, block, 0, stream>>>(
          ...
          batch_size, in_c, in_h, in_w, out_c, grad_output.h, grad_output.w, k,
          stride, padding);
      CUDA_CHECK(cudaGetLastError());
    }
