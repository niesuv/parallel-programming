/**
 * @file main.cpp
 * @brief CPU Autoencoder Training Entry Point
 *
 * Main program for training the autoencoder on CPU.
 * Supports OpenMP parallelization for multi-threaded training.
 *
 * Usage:
 *   ./cpu_train <data_dir> <epochs> <batch_size> <lr> <log_path> <max_images>
 * <use_openmp>
 *
 * Features:
 * - CIFAR-10 dataset loading
 * - Training loop with progress logging
 * - Weight saving to binary file
 * - CSV export for loss curves
 */
...
public:
  ...
  void log_config(int epochs, int batch_size, float lr,
                  const std::string &data_dir, bool use_openmp,
                  int max_images) {
    if (!log_file_)
      return;
  ...
  void log_batch(int batch, int num_batches, float loss,
  ...
  void log_epoch_end(int epoch, int total_epochs, float avg_loss,
                     double epoch_time_sec, bool is_best, float best_loss) {
    if (log_file_) {
      log_file_ << "Epoch " << epoch << "/" << total_epochs;
      if (is_best)
        log_file_ << " [BEST]";
      log_file_ << "\n";
      log_file_ << "  Avg Loss: " << std::fixed << std::setprecision(6)
                << avg_loss << "\n";
      log_file_ << "  Best Loss: " << std::setprecision(6) << best_loss << "\n";
      log_file_ << "  Time: " << std::setprecision(2) << epoch_time_sec
                << " seconds\n";
  ...
  void log_training_complete(int total_epochs, float best_loss, float avg_loss,
                             double total_time) {
    if (!log_file_)
      return;
    log_file_ << "=== Training Complete ===\n";
    log_file_ << "Total Epochs: " << total_epochs << "\n";
    log_file_ << "Best Loss: " << std::fixed << std::setprecision(6)
              << best_loss << "\n";
    log_file_ << "Average Loss: " << std::setprecision(6) << avg_loss << "\n";
    log_file_ << "Total Training Time: " << std::setprecision(2) << total_time
              << " seconds\n";
  }
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    logger.log_config(epochs, batch_size, learning_rate, data_dir, use_openmp,
                      max_train_images);
    ...
    if (train.num_images == 0) {
      std::cerr << "No training images loaded. Check data directory: "
                << data_dir << std::endl;
      logger.log("ERROR: No training images loaded. Check data directory: " +
                 data_dir);
      return 1;
    }
    ...
    if (max_train_images > 0 && max_train_images < effective_train) {
      ...
      logger.log("Limited training to " + std::to_string(effective_train) +
                 " images");
    }
    ...
    if (num_batches == 0) {
      std::cerr << "Not enough training images for batch_size = " << batch_size
                << std::endl;
      logger.log("ERROR: Not enough training images for batch_size = " +
...
#else
      std::cout
          << "Warning: OpenMP requested (use_openmp=1) but program was not "
             "compiled with -fopenmp. Running single-threaded."
          << std::endl;
      logger.log("WARNING: OpenMP requested but not compiled. Running "
                 "single-threaded.");
#endif
    } else {
    ...
    for (int epoch = 0; epoch < epochs; ++epoch) {
      ...
      for (int b = 0; b < num_batches; ++b) {
        ...
        const std::size_t image_size =
            static_cast<std::size_t>(CIFAR10Dataset::IMAGE_SIZE);
#ifdef _OPENMP
#pragma omp parallel for schedule(static)
        ...
        for (int i = 0; i < batch_size; ++i) {
          ...
          const float *__restrict__ src =
              &train.images[static_cast<std::size_t>(img_idx) * image_size];
          float *__restrict__ input_ptr =
              input.data.data() + static_cast<std::size_t>(i) * image_size;
          float *__restrict__ target_ptr =
              target.data.data() + static_cast<std::size_t>(i) * image_size;
      ...
      logger.log_epoch_end(epoch + 1, epochs, epoch_loss, seconds, is_best,
                           best_loss);

      logger.write_csv(epoch + 1, epoch_loss, seconds);

      std::cout << "Epoch " << (epoch + 1) << "/" << epochs
                << " - loss: " << epoch_loss << " - time: " << seconds << " s"
    ...
    if (!model.save_weights(weights_path)) {
      std::cerr << "Warning: failed to save weights to " << weights_path
                << std::endl;
