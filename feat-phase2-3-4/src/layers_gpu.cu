/**
 * @file layers_gpu.cu
 * @brief GPU Layer Implementations (Naive Version)
 *
 * This file contains the basic CUDA implementations for neural network layers.
 * These are straightforward implementations without advanced optimizations.
 *
 * Implemented layers:
 * - GPUTensor4D: GPU memory tensor with NCHW layout
 * - GPUConv2DLayer: Basic 2D convolution
 * - GPUReLULayer: ReLU activation
 * - GPUMaxPool2DLayer: Max pooling
 * - GPUUpSample2DLayer: Nearest-neighbor upsampling
 * - gpu_mse_loss: MSE loss with parallel reduction
 *
 * For optimized versions with shared memory and warp shuffle, see
 * layers_gpu_opt.cu
 */
...
void GPUTensor4D::copy_from_host_async(const float *h_data,
                                       cudaStream_t stream) {
  if (d_data && size() > 0) {
...
void GPUConv2DLayer::forward(const GPUTensor4D &input, GPUTensor4D &output,
...
void GPUConv2DLayer::forward_fused_relu(const GPUTensor4D &input,
  ...
  gpu_conv2d_relu_forward_opt(input, d_weights_, d_bias_, output, in_c_, out_c_,
                              k_, stride_, padding_, stream);
#else
  ...
  // Apply ReLU in-place
  {
    ...
    relu_forward_kernel<<<grid_size, block_size, 0, stream>>>(
        output.d_data, output.d_data, total);
    CUDA_CHECK(cudaGetLastError());
  }
#endif
}
...
void GPUConv2DLayer::backward_fused_relu(const GPUTensor4D &input,
  ...
  gpu_conv2d_relu_backward_data_opt(grad_output, d_weights_, input, grad_input,
                                    input.n, in_c_, input.h, input.w, out_c_,
                                    k_, stride_, padding_, stream);
  ...
  gpu_conv2d_backward_weights_opt(input, grad_output, d_grad_weights_,
                                  ...
                                  padding_, stream);
#else
  // Fallback: separate ReLU and Conv backward
  GPUTensor4D grad_relu(grad_output.n, grad_output.c, grad_output.h,
                        grad_output.w);
  {
    ...
    relu_backward_kernel<<<grid_size, block_size, 0, stream>>>(
        input.d_data, grad_output.d_data, grad_relu.d_data, total);
    CUDA_CHECK(cudaGetLastError());
  }
...
void GPUReLULayer::forward(const GPUTensor4D &input, GPUTensor4D &output,
                           cudaStream_t stream) const {
...
void GPUMaxPool2DLayer::forward(const GPUTensor4D &input, GPUTensor4D &output,
                                cudaStream_t stream) const {
  int out_h = get_output_h(input.h);
  int out_w = get_output_w(input.w);
...
void GPUUpSample2DLayer::forward(const GPUTensor4D &input, GPUTensor4D &output,
                                 cudaStream_t stream) const {
  int out_h = get_output_h(input.h);
  int out_w = get_output_w(input.w);
...
__global__ void mse_grad_kernel(const float *__restrict__ output,
  ...
  size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < n)
    grad_output[idx] = 2.0f * scale * (output[idx] - target[idx]);
}
...
float gpu_mse_loss_with_grad(const GPUTensor4D &output,
  ...
  float scale =
      1024.0f / static_cast<float>(n); // scale để loss và grad đồng bộ
  int block_size = 256;
