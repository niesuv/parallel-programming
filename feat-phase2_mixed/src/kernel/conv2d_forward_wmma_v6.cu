...
// 4 warps = 128 threads (same as V5)
constexpr int WARPS_PER_BLOCK = 4;
constexpr int THREADS_PER_BLOCK = WARPS_PER_BLOCK * 32;  // 128

// Tile sizes (same as V5)
constexpr int TILE_H = 8;
constexpr int TILE_W = 8;

// Output channels per block (same as V5)
...
// ============================================================================
// Single-buffered (no double buffering to save memory):
//   Input:  10 * 10 * 16 * 2 bytes = 3,200 bytes = 3.1 KB
//   Weight: 9 * 2 * 16 * (16+8) * 2 bytes = 13,824 bytes = 13.5 KB  
//   A workspace: 4 * 16 * (16+8) * 2 bytes = 3,072 bytes = 3.0 KB
//   Output: 4 * 16 * (16+4) * 4 bytes = 5,120 bytes = 5.0 KB
//   Total: ~25 KB (well under 48 KB)
//
// Double-buffered would be:
//   Input:  2 * 3.2 KB = 6.4 KB
//   Weight: 2 * 13.5 KB = 27 KB
//   Total: ~41 KB (still fits!)

// Use single buffering for simplicity and lower memory
...
conv2d_fp16_wmma_v6_kernel(
    ...
    // ========================================================================
    // SHARED MEMORY - Optimized layout for bank-conflict-free access
    // ========================================================================
    // Input: [ih][iw][ic] - spatial locality for 3x3 window access
    __shared__ __align__(16) half s_input[INPUT_TILE_H][INPUT_TILE_W][IC_PER_ITER + SMEM_PAD];
    
    // Weight: [kpos][oc_tile][oc][ic] for col_major B matrix
    __shared__ __align__(16) half s_weight[9][OC_TILES_PER_BLOCK][WMMA_N][WMMA_K + SMEM_PAD];
    
    // A matrix workspace per warp
    ...
    // ========================================================================
    // MAIN LOOP - Iterate over input channels
    // ========================================================================
    for (int c_base = 0; c_base < C; c_base += WMMA_K) {
        
        // --------------------------------------------------------------------
        // Load input tile - coalesced access pattern
        // --------------------------------------------------------------------
        {
        ...
        // --------------------------------------------------------------------
        // Load weights - transposed for col_major B matrix
        // --------------------------------------------------------------------
        {
        ...
        // --------------------------------------------------------------------
        // Compute - process all 9 kernel positions
        // --------------------------------------------------------------------
        #pragma unroll
        for (int kh = 0; kh < 3; ++kh) {
            ...
            for (int kw = 0; kw < 3; ++kw) {
                ...
                // Build A matrix in shared memory
                #pragma unroll
                for (int i = 0; i < 8; ++i) {
                ...
                // Load B fragments and compute (on-demand to reduce register pressure)
                #pragma unroll
                for (int t = 0; t < OC_TILES_PER_BLOCK; ++t) {
...
__global__ void __launch_bounds__(THREADS_PER_BLOCK, 4)
conv2d_fp16_wmma_v5_kernel(
    const half* __restrict__ input,   // [N, H, W, C] - NHWC
    const half* __restrict__ weight,  // [K, C, 3, 3] - KCRS
    const half* __restrict__ bias,    // [K]
    half* __restrict__ output,        // [N, H, W, K] - NHWK
    const int N, const int H, const int W, const int C, const int K){
    const int tid = threadIdx.x;
    const int warp_id = tid / 32;
    const int lane_id = tid % 32;
    ...
    // --------------------------------------------------------------------
    // Shared memory (aligned for WMMA)
    // --------------------------------------------------------------------
    __align__(16) __shared__ half s_input[INPUT_TILE_H][INPUT_TILE_W][IC_PER_ITER + SMEM_PAD];

    // FIX #1: Transposed weight layout for col_major B
    // [kpos][oc_tile][oc][ic]
    __align__(16) __shared__ half s_weight[9][OC_TILES_PER_BLOCK][WMMA_N][WMMA_K + SMEM_PAD];

    __align__(16) __shared__ half s_A[WARPS_PER_BLOCK][WMMA_M][WMMA_K + SMEM_PAD];
    ...
    for (int c_base = 0; c_base < C; c_base += WMMA_K) {
        ...
        // Load input tile
        // ---------------------------
        const int input_elems = INPUT_TILE_H * INPUT_TILE_W * WMMA_K;
        for (int idx = tid; idx < input_elems; idx += THREADS_PER_BLOCK) {
            const int ic = idx % WMMA_K;
            const int iw = (idx / WMMA_K) % INPUT_TILE_W;
            const int ih = idx / (WMMA_K * INPUT_TILE_W);
        ...
        // ---------------------------
        const int weight_elems = 9 * OC_TILES_PER_BLOCK * WMMA_K * WMMA_N;
        for (int idx = tid; idx < weight_elems; idx += THREADS_PER_BLOCK) {
            const int oc = idx % WMMA_N;
            const int ic = (idx / WMMA_N) % WMMA_K;
            const int oc_tile = (idx / (WMMA_N * WMMA_K)) % OC_TILES_PER_BLOCK;
            const int kpos = idx / (WMMA_N * WMMA_K * OC_TILES_PER_BLOCK);
            ...
            // FIX #1: [oc][ic] layout for col_major
            s_weight[kpos][oc_tile][oc][ic] = w;
        }
        ...
        for (int kw = 0; kw < 3; ++kw) {
            ...
            for (int i = 0; i < 8; ++i) {
                ...
                // FIX #2: zero invalid spatial rows
                if (spatial_idx < TILE_H * TILE_W &&
                    out_h_base + local_h < H &&
                    out_w_base + local_w < W) {
                    s_A[warp_id][m][k] =
                        s_input[local_h + kh][local_w + kw][k];
                } else {
                    s_A[warp_id][m][k] = __float2half(0.0f);
                }
            }
            ...
            #pragma unroll
            for (int t = 0; t < OC_TILES_PER_BLOCK; ++t)
                mma_sync(acc[t], a_frag, b_frag[kpos][t], acc[t]);
    ...
    #pragma unroll
    for (int t = 0; t < OC_TILES_PER_BLOCK; ++t) {
        store_matrix_sync(&s_out[warp_id][0][0], acc[t],
                        WMMA_N + 4, mem_row_major);
        __syncwarp();
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            if (gh < H && gw < W && gk < K) {
                output[((batch_idx * H + gh) * W + gw) * K + gk] =
                    __float2half(s_out[warp_id][m][n] + __half2float(bias[gk]));
            }
        }
        __syncwarp();
    }
...
// ============================================================================
__global__ void __launch_bounds__(THREADS_PER_BLOCK, 4)
conv2d_relu_fp16_wmma_v5_kernel(
    const half* __restrict__ input,
    const half* __restrict__ weight,
    const half* __restrict__ bias,
    half* __restrict__ output,
    ...
    // --------------------------------------------------------------------
    __align__(16) __shared__ half s_input[INPUT_TILE_H][INPUT_TILE_W][WMMA_K + SMEM_PAD];

    // FIX #1: [kpos][oc_tile][oc][ic]  (col_major B)
    __align__(16) __shared__ half s_weight[9][OC_TILES_PER_BLOCK][WMMA_N][WMMA_K + SMEM_PAD];
    ...
    for (int t = 0; t < OC_TILES_PER_BLOCK; ++t) {
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            if (gh < H && gw < W && gk < K) {
                float conv_val =
                    s_out[warp_id][m][n] +
                    __half2float(bias[gk]);

                const size_t out_idx =
                    ((batch_idx * H + gh) * W + gw) * K + gk;

                if (conv_output)
                    conv_output[out_idx] = __float2half(conv_val);

                output[out_idx] =
                    __float2half(fmaxf(conv_val, 0.0f));
            }
        }
        __syncwarp();
    }
...
__global__ void conv2d_fp16_reference_kernel(
    ...
    for (int c = 0; c < C; ++c) {
        for (int kh = 0; kh < 3; ++kh) {
            for (int kw = 0; kw < 3; ++kw) {
                const int h_in = h_out + kh - 1;  // -1 for padding
                const int w_in = w_out + kw - 1;
                
                if (h_in >= 0 && h_in < H && w_in >= 0 && w_in < W) {
                    // input[n, h, w, c] in NHWC
                    float in_val = __half2float(input[((n * H + h_in) * W + w_in) * C + c]);
                    // weight[k, c, kh, kw] in KCRS
...
void launch_conv2d_fp16_wmma_optimized(
    ...
    conv2d_fp16_wmma_v5_kernel<<<grid, block, 0, stream>>>(
        input, weight, bias, output, N, H, W, C, K
    );
}
...
void launch_conv2d_relu_fp16_wmma_optimized(
    ...
    conv2d_relu_fp16_wmma_v5_kernel<<<grid, block, 0, stream>>>(
        input, weight, bias, output, conv_output, N, H, W, C, K
    );
}
